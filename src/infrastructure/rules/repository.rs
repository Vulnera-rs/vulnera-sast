...
impl PostgresRuleRepository {
    ...
    fn row_to_rule(row: SastRuleRow) -> Result<Rule, RuleRepositoryError> {
        let pattern = Pattern::TreeSitterQuery(row.query);
...
impl SastRuleRepository for PostgresRuleRepository {
    ...
    async fn upsert_tree_sitter_rule(&self, rule: &Rule) -> Result<(), RuleRepositoryError> {
        let query = match &rule.pattern {
            Pattern::TreeSitterQuery(q) => q,
            _ => {
                return Err(RuleRepositoryError::InvalidData(
                    "Only TreeSitterQuery patterns are supported".to_string(),
                ));
            }
        };
    ...
    async fn get_updated_rules_since(
        ...
        debug!(
            rules_updated = rules.len(),
            "Found updated rules since {}", since
        );
    ...
    async fn get_latest_update_time(&self) -> Result<Option<DateTime<Utc>>, RuleRepositoryError> {
        let result = sqlx::query_scalar::<_, Option<DateTime<Utc>>>(
            r#"
            SELECT MAX(updated_at) FROM sast_rules
            "#,
        )
        .fetch_one(&self.pool)
        .await?;
...
impl RuleHotReloader {
    ...
    #[instrument(skip(self))]
    pub async fn check_for_updates(&self) -> Result<Vec<Rule>, RuleRepositoryError> {
        let last = *self.last_check.read().await;
        let rules = self.repository.get_updated_rules_since(last).await?;

        if !rules.is_empty() {
            info!(rules_updated = rules.len(), "Rules updated since {}", last);
        }
            ...
            loop {
                ...
                match reloader.check_for_updates().await {
                    Ok(rules) => {
                        if !rules.is_empty() {
                            debug!(
                                "Hot-reload detected {} tree-sitter rule updates",
                                rules.len()
                            );
